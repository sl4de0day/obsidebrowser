...
// Obside security fix (C24/C164): messages that expose the account/token/license bridge
// or run SpecialMessageActions must only be honored when they actually originate from a
// trusted first-run document. The platform already scopes this actor, but this is an
// explicit defense-in-depth sender-principal check on the sensitive handlers.
const OBSIDE_SENSITIVE_MESSAGES = new Set([
  "AWPage:SPECIAL_ACTION",
  "AWPage:HANDLE_CAMPAIGN_ACTION",
  "AWPage:OBSIDE_MAKE_SOLE_TAB",
  "AWPage:OBSIDE_GET_ACCOUNT",
  "AWPage:OBSIDE_OPEN_ACCOUNT",
  "AWPage:OBSIDE_BUY_PREMIUM",
  "AWPage:OBSIDE_ACTIVATE_LICENSE",
]);
const OBSIDE_TRUSTED_ABOUT = ["about:welcome", "about:home", "about:newtab"];
...
export class AboutWelcomeParent extends JSWindowActorParent {
   ...
   */
  // Obside security fix (C24/C164): confirm the calling document is a trusted about:
  // first-run page before honoring account/token/license/action messages.
  _isObsideTrustedCaller() {
    try {
      const spec = this.manager?.documentURI?.spec || "";
      return OBSIDE_TRUSTED_ABOUT.some(
  ...
  async onContentMessage(type, data, browser) {
    ...
    if (OBSIDE_SENSITIVE_MESSAGES.has(type) && !this._isObsideTrustedCaller()) {
      lazy.log.warn(`Obside: refusing sensitive message ${type} from untrusted document`);
      return undefined;
    }
    switch (type) {
      ...
      case "AWPage:OBSIDE_MAKE_SOLE_TAB":
        return lazy.ObsideWelcomeAccount.makeSoleTab(browser);
      case "AWPage:OBSIDE_GET_ACCOUNT":
        return lazy.ObsideWelcomeAccount.getAccount();
      case "AWPage:OBSIDE_OPEN_ACCOUNT":
        return lazy.ObsideWelcomeAccount.openAccountPage(data?.target, browser);
      case "AWPage:OBSIDE_BUY_PREMIUM":
        return lazy.ObsideWelcomeAccount.buyPremium(browser);
      case "AWPage:OBSIDE_ACTIVATE_LICENSE":
        return lazy.ObsideWelcomeAccount.activateLicense(data?.key);
