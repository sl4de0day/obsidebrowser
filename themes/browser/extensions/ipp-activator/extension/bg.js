...
class IPPAddonActivator {
  ...
  async #uninit() {
    ...
    // Obside optimization: these two independent allSettled batches were awaited
    // sequentially; run them concurrently.
    await Promise.all([
      Promise.allSettled(
  ...
  async #loadAndRebuildBreakages() {
    ...
    // Obside optimization: these two independent getters were awaited sequentially; run
    // them concurrently. Each keeps its own []-fallback + warning on failure.
    const [dynamicTab, dynamicWr] = await Promise.all([
  ...
  async #maybeNotify(tab, breakages, url) {
    ...
    // Obside optimization: match the (local, in-memory) breakage list BEFORE the
    // hasExclusion() IPC. Most URLs match no breakage, so this returns early without the
    // extra round-trip to the parent process. Breakage match and exclusion are
    // independent gates, so reordering them does not change behavior.
