...
class ConditionFactory {
  ...
  static async run(conditionDesc, context = {}) {
    ...
    // Obside optimization: create() is synchronous, so drop the await (removes a needless
    // microtask hop). A synchronous throw for an unknown type still surfaces as a
    // rejection of run()'s promise, so behavior is unchanged.
    const condition = factory.create(conditionDesc);
