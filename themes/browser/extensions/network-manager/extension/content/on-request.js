...
export class OnRequest {
  ...
  static {
    ...
    // Obside security fix (C15): fail CLOSED during cold start. The proxy listener is
    // registered below BEFORE init() has loaded the stored config, so this.mode is the
    // default 'disable'. Any request in that sub-second window would otherwise be sent
    // DIRECT (leaking the real IP, e.g. session-restore tabs) while the UI independently
    // reports "Protected". We gate every decision on `ready`; until init() completes we
    // HOLD the request (proxy.onRequest accepts an async/Promise return) instead of
    // returning direct. If init() never completes we intentionally keep holding — for a
    // VPN, no traffic is safer than leaked traffic.
    this.ready = false;
  ...
  static init(pref) {
    ...
    // Obside security fix (C15): config is now loaded — release any requests held
    // during cold start so they are decided against the real proxy config.
    this.ready = true;
    this._resolveReady && this._resolveReady();
  }

  static process(e) {
    // Obside security fix (C15): don't decide with a not-yet-initialized config.
    if (!this.ready) {
      return this.readyPromise.then(() => this._decide(e));
    }
    return this._decide(e);
  }
  ...
  // Obside optimization: cache compiled include/exclude/passthrough/tabProxy RegExps by
  // pattern string. They were recompiled from config strings on EVERY request; patterns
  // are stable between config changes and the 'i'-only regexes are stateless (.test with
  // no 'g' flag), so reusing a cached compile is behavior-preserving.
  static _reCache = new Map();
  static _re(pattern) {
    let re = this._reCache.get(pattern);
    if (re === undefined) {
      re = new RegExp(pattern, 'i');
      this._reCache.set(pattern, re);
    }
    return re;
  }
  ...
  static processTabProxy(tabId, url, e) {
    ...
    const match = arr => arr.some(i => this._re(i).test(url));
  ...
  static bypass(url) {
    switch (true) {
      // global passthrough
      case this.passthrough.some(i => this._re(i).test(url)):
