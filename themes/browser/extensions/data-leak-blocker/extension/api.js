...
this.dataAbuseDetection = class extends ExtensionAPI {
  ...
  onStartup() {
    ...
    ExtensionParent.browserStartupPromise
      ...
      // Obside optimization/hardening: log a startup failure with context instead of
      // leaving it as an unhandled promise rejection.
      .catch(err => {
        console.error("Data Leak Blocker initialization failed", err);
      });
  }
  ...
  processHttpOnModifyRequest(channel) {
    // Obside optimization: read channel.loadInfo once (it was accessed twice below).
    ...
    // Obside optimization: this observer runs for EVERY http(s) request. Apply the cheap,
    // most-selective filter (blocklisted host) before reading principals, so the common
    // case (host not on the blocklist) returns after a single Set lookup and never touches
    // the principal getters. These are independent gates, so reordering is behavior-neutral.
